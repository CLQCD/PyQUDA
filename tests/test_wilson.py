...
from pyquda.enum_quda import (
    QudaDagType, QudaDiracFieldOrder, QudaDslashType, QudaGammaBasis, QudaGaugeFieldOrder, QudaGaugeFixed,
    QudaInverterType, QudaLinkType, QudaMassNormalization, QudaMatPCType, QudaParity, QudaPrecision,
...
for spin in range(Ns):
    for color in range(Nc):
        ...
        # data[:, St, :, :, :, spin, color] = 1

        # tmp.even = b.even
        # quda.dslashQuda(tmp.odd_ptr, tmp.even_ptr, quda_inv_param, QudaParity.QUDA_ODD_PARITY)
        # tmp.odd *= 2 * kappa
        # tmp.odd = b.odd + 0.5 * tmp.odd
        ...
        # tmp.even = b.even + 0.5 * tmp.even
        # x.even = tmp.even
        # x.even *= 2 * kappa

        tmp.even = 2 * kappa * b.even
        tmp.odd = 2 * kappa * b.odd
        quda.dslashQuda(tmp2.odd_ptr, tmp.even_ptr, quda_inv_param, QudaParity.QUDA_ODD_PARITY)
        tmp2.odd *= 2 * kappa
        tmp2.odd *= -0.5
        tmp.odd = tmp.odd - tmp2.odd
        quda.invertQuda(x.odd_ptr, tmp.odd_ptr, quda_inv_param)
        # x.odd /= 2 * kappa  # QUDA_ASYMMETRIC_MASS_NORMALIZATION
        # x.odd /= 4 * kappa**2  # QUDA_MASS_NORMALIZATION
...
propagator_chroma = np.fromfile("pt_prop_0", ">c16", offset=8).reshape(Vol, Ns, Ns, Nc, Nc)
print(propagator[:2, 0, 0, 0, 0])
print(propagator_chroma[:2, 0, 0, 0, 0])
print(propagator[Vol // 2:Vol // 2 + 2, 0, 0, 0, 0])
print(propagator_chroma[Vol // 2:Vol // 2 + 2, 0, 0, 0, 0])
