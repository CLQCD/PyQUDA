...
for i in range(100):
    ...
    for step in range(steps):
        ...
        # print(step)
        hmc.computeHISQForce(vartheta_ * dt, noise, -(kappa**2), -kappa * csw / 8)
        hmc.updateGaugeField(rho_ * dt)
        hmc.computeGaugeForce(lambda_ * dt, force, flengths, fcoeffs, num_fpaths, max_length - 1)
        hmc.computeHISQForce(lambda_ * dt, noise, -(kappa**2), -kappa * csw / 8)
        ...
        hmc.computeHISQForce((0.5 - (lambda_ + vartheta_)) * dt, noise, -(kappa**2), -kappa * csw / 8)
