    ...
    for n, mu in zip([1, 1], [-X, T]):
        ...
        for spin in range(4):
            for color in range(3):
                propagator_covdev.setFermion(dirac.covDev(propagator.getFermion(spin, color), mu), spin, color)
        cp.cuda.runtime.deviceSynchronize()
        e = perf_counter()
        ...
        for spin in range(4):
            for color in range(3):
                multi_fermion_covdev[spin * 3 + color] = dirac.covDev(multi_fermion[spin * 3 + color], mu)
        cp.cuda.runtime.deviceSynchronize()
        e = perf_counter()
